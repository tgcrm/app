...
const UserInterface = () => {
  ...
  useEffect(() => {
    ...
    if (AuthUser) {
      ...
      setLeads(tempLead);
      const newLeads = tempLead.filter((item) => {
        return dayjs(item.date, "DD-MM-YYYY").isBetween(
      ...
      if (response) {
        try {
          const leadRes = await fetch(
            "https://tgcrm-root-api.vercel.app/update-status-leads",
            {
              method: "POST",
              body: JSON.stringify(newdata),
              headers: {
  ...
  const FilterTab = () => {
    return (
      <div className="w-12/12  bg-white/30 backdrop-blur-lg rounded  shadow-lg mt-4 mb-4 mx-2  ">
  ...
  const StatusComp = (props) => (
    <div
      className={`flex flex-col w-44 justify-start  m-1 rounded  bg-${props.color}-600  `}
    >
      ...
      <div className="flex flex-col justify-between items-center text-lg font-sans font-bold p-1  w-full ">
        ...
        >
          {/* {props.name === "active"
            ? NewStatus.map(
                (item) =>
                  item.admin_count === "yes" && (
                    <StatusComp
                      name={leadStatusCounts[item.name]}
                      value={200}
                      color={props.color}
                    />
                  )
              )
            : props.name === "followups"
            ? NewStatus.map(
                (item) =>
                  item.followup_count === "yes" && (
                    <StatusComp
                      name={item.name}
                      value={leadStatusCounts[item.name]}
                      color={props.color}
                    />
                  )
              )
  ...
  return (
    <div
      id="container"
      className=" min-h-screen max-h-screen w-12/12 bg-gradient-to-r from-Primary to-Secondary overflow-hidden"
    >
      {contextHolder}
      {topMenu}
      {/* ////filtertab//// */}
      ...
      {selectedView === "call" ? (
        <>
          ...
          >
            {TableData.map((lead) => (
              <div
                className={`flex w-12/12 rounded-md border-2   shadow-lg mt-4 mb-4  mx-2 
                   border-white`}
              >
                {/* Info column*/}
                <div className="flex flex-col rounded-md justify-center items-center  text-lg font-sans  p-1   border-white  w-6/12 ">
                  <div className="flex justify-start  items-center py-1 w-full pl-2 text-white">
                    {toCamelCase(lead.name)}
                    {
                      <div
                        style={{ backgroundColor: `${lead.tagcolor}` }}
                        className={`ml-4 w-3 h-3 rounded-full `}
                      ></div>
                    }
                  </div>
                  <div className="flex justify-start items-center py-1 w-full  pl-2  mb-1 text-white">
                    {lead.mobile}
                  </div>
                  <button
                    onClick={() => handleResponseSubmit(lead._id, lead.status)}
                    name=""
                    id=""
                    className=" flex justify-center  bg-white/30 backdrop-blur-lg items-center py-2 w-full  rounded-md my-1  text-white"
                  >
                    Submit
                  </button>
                </div>
                ...
                {/* Response column*/}
                <div className="flex flex-col justify-evenly items-center gap-2 text-xl font-sans  p-1   w-4/12">
                  <select
                    name=""
                    id=""
                  ...
                  <input
                    ...
                    name=""
                    id=""
                    className="flex justify-center items-center h-9 pl-2  text-sm w-full rounded-md  bg-slate-300"
                    onChange={handleResponseComment}
                  />
                </div>

                {/* Call Buttons column*/}

                <div className="flex flex-col justify-evenly items-center gap-2 text-xl font-sans font-bold p-1   w-2/12">
                  <button className="flex justify-center items-center text-4xl text-white p-1 my-1 h-full w-full rounded-md  bg-blue-500">
                    <FaPhone />
                  </button>
                  <button className="flex justify-center items-center text-4xl text-white p-1 my-1 h-full w-full rounded-md  bg-green-500">
                    <FaWhatsapp color="" />
                  </button>
                </div>
              </div>
     ...
     "
            >
              ...
              <div className="flex flex-col justify-center items-center text-lg font-sans font-bold p-1  w-full ">
                <div className="flex justify-center items-center py-1 w-full rounded-md my-1  text-white">
                  Total leads Assigned : 100
                </div>
              </div>
              {/* Active */}
            </div>
            {Object.entries(Status_count).map(([key, value]) => (
              <AnalyticsComp name={key} color={"red"} value={value} />
            ))}
          </div>
        </>
      )}
      ...
      {/* Action Comp */}
      <div className="w-12/12  bg-gray-200 rounded  shadow-lg ">
